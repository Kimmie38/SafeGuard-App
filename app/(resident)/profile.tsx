...
const ROWS: {
  ...
  { icon: "person-outline", label: "Personal details", accent: "navy" },
  { icon: "location-outline", label: "My region in Jos", accent: "amber" },
  { icon: "document-text-outline", label: "My reports", accent: "teal" },
  { icon: "shield-outline", label: "Privacy & safety", accent: "coral" },
  { icon: "notifications-outline", label: "Notification preferences", accent: "navy" },
  { icon: "help-circle-outline", label: "Help & support", accent: "amber" },
];
...
export default function Profile() {
  const { userName, userRegion, reports, logout } = useApp();

  const mine = reports.filter((r) => r.reporter === userName);
        ...
        <View className="px-6 -mt-10 mb-6">
          <View className="flex-row bg-card border border-hairline rounded-2xl overflow-hidden">
            ...
            <View className="flex-1 items-center py-4">
              <Text className="font-display-bold text-xl text-coral">
